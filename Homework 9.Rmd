...
optValueAppr <- function(n, r, sigma, S0, K, tgrid) {
    ...
    ## result
    return(c(pAri, ST, pStd, pGeo) * exp(-r * TT))
}
```
...
correlations <- c(cor(sim[1,], sim[2,]), cor(sim[1,], sim[3,]), cor(sim[1,], sim[4,]))
...
correlations
```
It can be seen that as $\sigma$ increases, positive correlation between the asset and arithmetic asian option declines. But the correlation between Geometric and Arithmetic Asian stocks increase.
...
correlations
```
It can be seen that as T increases, positive correlations deacrease. 
